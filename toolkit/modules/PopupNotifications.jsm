  ...
  show: function PopupNotifications_show(browser, id, message, anchorID,
    ...
    if (isActiveBrowser) {
      if (isActiveWindow) {
        ...
        this._updateAnchorIcon(notifications, this._getAnchorsForNotifications(
...
////////////////////////////////////////////////////////////////////////////////
  ...
  _update: function PopupNotifications_update(notifications, anchors = new Set(), dismissShowing = false) {
    ...
    if (haveNotifications) {
      ...
      } else if (anchorElements.size) {
        this._updateAnchorIcon(notifications, anchorElements);
      }

      // Also filter out notifications that are for a different anchor.
      notificationsToShow = notificationsToShow.filter(function (n) {
        return anchorElements.has(n.anchorElement);
      });
    }
  ...
  _swapBrowserNotifications: function PopupNotifications_swapBrowserNoficications(ourBrowser, otherBrowser) {
    ...
    if (otherNotifications.length > 0)
      this._update(otherNotifications, this._getAnchorsForNotifications(otherNotifications));
    if (ourNotifications.length > 0)
      other._update(ourNotifications, this._getAnchorsForNotifications(ourNotifications));
  },
